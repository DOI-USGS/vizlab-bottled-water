...
get_shift <- function(region_abbr){
  albers <- '+proj=aea +lat_0=23 +lon_0=-96 +lat_1=29.5 +lat_2=45.5 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs +type=crs'
  switch(
    region_abbr,
    AK = list(scale = 0.40, shift = c(-165,-110), rotation_deg = 0, proj_str = "EPSG:3338"),
...
generate_single_oconus_sf <- function(region_abbrs, proj_str, states_shp) {
...
build_oconus_sf <- function(proj_str, states_shp) {
  ...
  oconus_sf_noshift <- generate_single_oconus_sf(unlist(region_abbr_list), proj_str, states_shp)
...
shift_sf <- function(obj_sf, rotation_deg, scale, shift, proj_str) {
  ...
  # get union of object before computing centroid to use in rotation and scaling
  # (necessary b/c of cases where nrow(obj_sf) > 1 and want to shift geometries together)
  obj_sfg_centroid <- st_centroid(st_union(obj_sfg))
  ...
  # get union of object before computing centroid to use in shift
  # (necessary b/c of cases where nrow(obj_sf) > 1 and want to shift geometries together)
  obj_sfg_centroid_new <- st_centroid(st_union(obj_sfg_rot_scaled))
  ...
  obj_sfg_shifted_chunked <- split(obj_sfg_shifted, ceiling(seq_along(obj_sfg_shifted) / 2))
...
apply_shifts_to_sites <- function(sites_sf, proj_str, states_shp) {
  ...
  oconus_site_sf_shifted <- purrr::map_df(
    # list PR and VI as single item so that mapped and shifted together
...
shift_points_sf <- function(pts_sf, rotation_deg, scale, shift, proj_str, ref_sf = NULL) {
  ...
  if(!is.null(ref_sf)) {
    ...
    # get union of object before computing centroid to use in rotation and scaling
    # (necessary b/c of cases where nrow(ref_sf) > 1 and want to shift geometries together)
    scaling_center <- st_coordinates(st_centroid(st_union(ref_sf_transf)))
  } else {
    scaling_center <- find_pts_centroid(pts_sfg_rot) # pts_sfg_rot doesn't exist...should be pts_sfg?
  }
